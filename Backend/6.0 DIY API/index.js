...
app.put("/jokes/:id", (req, res) => {
  ...
  const jokeIx = jokes.findIndex((joke) => joke.id === updatedId);
  jokes[jokeIx] = updatedJoke;
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const jokeIx = jokes.findIndex((joke) => joke.id === id);
  jokes[jokeIx] = updatedJoke;
  console.log(jokes[jokeIx]);
  res.json(updatedJoke);
...
app.delete("/jokes/:id", (req, res) => {
  ...
  const jokeIx = jokes.findIndex((joke) => joke.id === id);
  if(jokeIx > -1){
    jokes.splice(jokeIx, 1);
    res.sendStatus(200);
  }else{
    res.status(404).json({
      error: `Joke with id: ${id} is not found`
    })
  }
